...
class SingleAdmin extends StatefulWidget {
  ...
  const SingleAdmin({Key? key, required this.uid, required this.communityId}) : super(key: key);
...
class _SingleAdminState extends State<SingleAdmin> {
  ...
  getData() async {
    ...
    try {
      ...
      for (Map hs in userData['roles']) {
        if ( hs.containsKey(widget.communityId) ) {
          role = hs[widget.communityId];
        }
      }
